...
/**
 * This maps primitive `int` vs `boolean`.
 */
public class DkIntBooleanArrayMap extends TheBaseSparseArray {
	...
	public DkIntBooleanArrayMap(int capacity) {
		if (capacity <= 0) {
			this.keys = DkEmptyArray.INT;
			this.values = DkEmptyArray.BOOLEAN;
		}
		else {
	...
	public int indexOfValue(boolean value) {
		final int size = this.size;
		int index = -1;
		for (boolean element : this.values) {
